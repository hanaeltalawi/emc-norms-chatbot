...
Usage: python model_comparison_report/standalone_model_comparison.py model_comparison_report/Translated_Norms.docx
"""
...
TEST_QUESTIONS = [
    ...
    {
        "id": "Q3",
        "question": "Which mobile service in the 852–894 MHz range has the lowest limits?",
        "expected_answer": "3G, with 26 dB(µV) peak and 6 dB(µV) mean."
    },
    {
        "id": "Q4",
    ...
    {
        "id": "Q6",
        "question": "Which analog service has the lowest quasi-peak limit?",
        "expected_answer": "MW and SW, both with 7 dB(µV)."
    },
    {
        "id": "Q7",
        "question": "Which service has the highest peak limit among digital broadcasting?",
        "expected_answer": "DTTB, DAB-L-Band, and SDARS, all at 32 dB(µV)."
    },
    {
        "id": "Q8",
        "question": "What are the peak and mean limits for 5G n71 (617–652 MHz)?",
        "expected_answer": "Peak limit is 32 dB(µV), mean limit is also 32 dB(µV)."
    },
    {
        "id": "Q9",
        "question": "At what RBW are 3G services in the 791–821 MHz band measured?",
        "expected_answer": "1 MHz."
    },
    {
        "id": "Q10",
        "question": "What is the minimum sweep time for MW band (0.53–1.8 MHz) at -6 dB RBW?",
        "expected_answer": "200 s/MHz."
    },
    {
        "id": "Q11",
        "question": "Which service uses a 100 ms/MHz minimum sweep time?",
        "expected_answer": "FM, TV Band I, TV Band III, and TV Band IV."
    }
]
...
class InstantModelTester:
    ...
    def test_all_models(self, docx_path: str, models_to_test: list = None, include_baseline: bool = True):
        """Test all models including baseline"""
        ...
        # Add baseline test
        if include_baseline:
            print(f"\n📊 Including BM25-only baseline comparison")
            baseline_result = self.test_bm25_baseline(TEST_QUESTIONS)
            if baseline_result:
                self.results['bm25_baseline'] = baseline_result
    ...
    def test_bm25_baseline(self, questions: list):
        """Test BM25-only baseline (simulates keyword search without LLM comprehension)"""
        print(f"\n🔍 Testing BM25-Only Baseline (keyword search simulation)...")
        
        try:
            baseline_results = {
                'model_name': 'bm25_baseline',
                'display_name': 'BM25-Only Baseline',
                'description': 'Keyword search without LLM comprehension',
                'questions': [],
                'total_time': 0,
                'valid_responses': 0,
                'failed_responses': 0,
            ...
            for i, qa in enumerate(questions, 1):
                ...
                # Use ONLY BM25 retrieval (no LLM)
                if self.hybrid_searcher and self.hybrid_searcher.bm25_index:
                    # Get top document using BM25 only
                    keyword_results = self.hybrid_searcher._get_keyword_results(qa['question'], k=1)
                    
                    if keyword_results:
                        # Simulate what a user would see - just the most relevant text chunk
                        best_doc, score = keyword_results[0]
                        answer = f"Most relevant document section:\n\n{best_doc.page_content}"
                        
                        # For fair comparison, limit answer length similar to LLM responses
                        if len(answer) > 500:
                            answer = answer[:500] + "..."
                    else:
                        answer = "No relevant document sections found for this query."
                else:
                    answer = "BM25 index not available."
                
                response_time = time.time() - start_time
                baseline_results['total_time'] += response_time
                
                # Different validation for baseline - just check if we found something
                is_valid = (answer and 
                        "not available" not in answer and 
                        "No relevant" not in answer)
                
                if is_valid:
                    baseline_results['valid_responses'] += 1
                else:
                    baseline_results['failed_responses'] += 1
                
                baseline_results['questions'].append({
                    'question': qa['question'],
                    'expected_answer': qa['expected_answer'],
                    'actual_answer': answer,
                    'response_time': response_time,
                    'valid': is_valid,
                    'context_used': 1 if is_valid else 0,
                    'context_preview': [answer[:100] + "..." ] if is_valid else []
                })
            ...
            # Calculate metrics
            total_questions = len(baseline_results['questions'])
            if total_questions > 0:
                baseline_results['avg_response_time'] = baseline_results['total_time'] / total_questions
                baseline_results['response_rate'] = (baseline_results['valid_responses'] / total_questions) * 100
            
            return baseline_results
            
        except Exception as e:
            print(f"❌ Error testing BM25 baseline: {e}")
            return None
...
class ReportGenerator:
    ...
    def generate_report(self, output_filename: str = None):
        ...
        if self.results:
            # Filter out baseline for
            llm_results = {k: v for k, v in self.results.items() if k != 'bm25_baseline'}
            baseline_results = self.results.get('bm25_baseline')
    ...
    The analysis reveals clear trade-offs between response quality, speed, and computational requirements. Larger models generally provide more accurate and detailed responses but require longer processing times, while smaller models offer faster responses with potentially lower accuracy for complex technical questions."""
        ...
        self.add_baseline_comparison_section(doc)
        ...
        doc.add_paragraph(
            ...
            "A score of 2 indicates an accurate and complete answer, directly supported by the document content; "
            "1.5 represents a correct answer that includes unnecessary or verbose details; "
            "1 indicates a partially correct or incomplete response; "
            "and 0.5 indicates accurate responses flagged as invalid due to instruction-following failures."
...
"""
    ...
    def add_baseline_comparison_section(self, doc):
        """Add dedicated baseline comparison analysis"""
        if 'bm25_baseline' not in self.results:
            return
        
        doc.add_heading('Baseline Comparison: BM25 vs. LLM-RAG', level=1)
        
        baseline = self.results['bm25_baseline']
        best_llm = max([r for k, r in self.results.items() if k != 'bm25_baseline'], 
                    key=lambda x: x.get('response_rate', 0))
        
        comparison_text = f"""
    This analysis compares the BM25 keyword-search baseline against the best-performing LLM-RAG system to demonstrate the value added by language comprehension.

    BASELINE (BM25-Only):
    • Returns the most relevant document chunk without comprehension
    • Simulates traditional "Ctrl+F" search behavior
    • Response Rate: {baseline.get('response_rate', 0):.1f}%

    BEST LLM-RAG SYSTEM ({best_llm['display_name']}):
    • Comprehends query intent and synthesizes information
    • Extracts and rephrases relevant information concisely
    • Response Rate: {best_llm.get('response_rate', 0):.1f}%

    PERFORMANCE DIFFERENCE: {best_llm.get('response_rate', 0) - baseline.get('response_rate', 0):.1f}% improvement

    QUALITATIVE DIFFERENCES:
    The BM25 baseline provides raw document excerpts that users must manually interpret, while the LLM-RAG system delivers direct, synthesized answers. This represents the fundamental difference between information retrieval and true question answering.
    """
